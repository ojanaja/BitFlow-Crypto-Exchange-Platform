...
@Component({
  ...
  template: `
    ...
    >
      <div
        class="p-4 border-b border-slate-200 bg-white/90 backdrop-blur shrink-0"
      >
      ...
      <div class="overflow-x-auto flex-1">
        <table class="w-full text-left text-sm border-collapse">
          ...
          <tbody class="divide-y divide-slate-100 overflow-y-auto">
            ...
            >
              <td
                class="py-3 px-4 text-slate-500 font-mono text-xs hidden sm:table-cell"
              >
                {{ order.timestamp | date : 'MMM d, HH:mm' }}
              </td>
              <td
                class="py-3 px-4 font-bold text-xs"
                [ngClass]="{
                  'text-emerald-600': order.type === 'BUY',
                  'text-rose-500': order.type === 'SELL'
                }"
              >
                <span
                  *ngIf="order.category === 'LIMIT'"
                  class="text-[10px] bg-slate-100 px-1 rounded mr-1 text-slate-500"
                  >LIMIT</span
                >
                {{ order.type }}
              </td>
              ...
              <td class="py-3 px-4 text-right text-slate-700 font-mono">
                {{ order.quantity }}
              </td>
              <td class="py-3 px-4 text-right text-slate-700 font-mono">
...
export class OrderHistoryComponent implements OnInit, OnChanges {
  ...
  filterOrders() {
    if (this.symbol) {
      this.displayedOrders = this.allOrders.filter(
        (o) => o.symbol.toUpperCase() === this.symbol?.toUpperCase()
      );
    } else {
