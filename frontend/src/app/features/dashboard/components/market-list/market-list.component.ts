        ...
        <table class="w-full text-left border-collapse">
          <thead class="bg-slate-950 sticky top-0 z-10">
            <tr class="text-slate-500 text-xs uppercase tracking-wider">
              <th class="py-3 px-4 w-10"></th> <!-- Star Column -->
              
              <th (click)="toggleSort('name')" class="py-3 px-4 font-semibold cursor-pointer hover:text-white transition-colors group select-none">
                  <div class="flex items-center gap-1">
                      Asset Pair
                      <span class="text-[10px] flex flex-col -space-y-1" [class.opacity-100]="sortColumn === 'name'" [class.opacity-30]="sortColumn !== 'name'">
                          <span [class.text-emerald-400]="sortColumn === 'name' && sortDirection === 'asc'">▲</span>
                          <span [class.text-emerald-400]="sortColumn === 'name' && sortDirection === 'desc'">▼</span>
                      </span>
                  </div>
              </th>

              <th (click)="toggleSort('price')" class="py-3 px-4 text-right font-semibold cursor-pointer hover:text-white transition-colors group select-none">
                  <div class="flex items-center justify-end gap-1">
                      Price
                      <span class="text-[10px] flex flex-col -space-y-1" [class.opacity-100]="sortColumn === 'price'" [class.opacity-30]="sortColumn !== 'price'">
                          <span [class.text-emerald-400]="sortColumn === 'price' && sortDirection === 'asc'">▲</span>
                          <span [class.text-emerald-400]="sortColumn === 'price' && sortDirection === 'desc'">▼</span>
                      </span>
                  </div>
              </th>

              <th (click)="toggleSort('volume')" class="py-3 px-4 text-right font-semibold cursor-pointer hover:text-white transition-colors group select-none hidden md:table-cell">
                  <div class="flex items-center justify-end gap-1">
                      Volume (24h)
                      <span class="text-[10px] flex flex-col -space-y-1" [class.opacity-100]="sortColumn === 'volume'" [class.opacity-30]="sortColumn !== 'volume'">
                          <span [class.text-emerald-400]="sortColumn === 'volume' && sortDirection === 'asc'">▲</span>
                          <span [class.text-emerald-400]="sortColumn === 'volume' && sortDirection === 'desc'">▼</span>
                      </span>
                  </div>
              </th>

              <th (click)="toggleSort('change')" class="py-3 px-4 text-right font-semibold cursor-pointer hover:text-white transition-colors group select-none">
                  <div class="flex items-center justify-end gap-1">
                      Change
                      <span class="text-[10px] flex flex-col -space-y-1" [class.opacity-100]="sortColumn === 'change'" [class.opacity-30]="sortColumn !== 'change'">
                          <span [class.text-emerald-400]="sortColumn === 'change' && sortDirection === 'asc'">▲</span>
                          <span [class.text-emerald-400]="sortColumn === 'change' && sortDirection === 'desc'">▼</span>
                      </span>
                  </div>
              </th>
            </tr>
          ...
          <tbody class="divide-y divide-slate-800">
            <tr *ngFor="let coin of coins" (click)="navigateToTrade(coin.name)" class="group hover:bg-slate-800/60 transition-colors cursor-pointer">
              <td class="py-4 px-4">
                  <button (click)="toggleWatchlist($event, coin)" class="text-slate-600 hover:text-yellow-400 transition-colors focus:outline-none">
                      <svg *ngIf="isInWatchlist(coin)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 text-yellow-400">
                          <path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" />
                      </svg>
                      <svg *ngIf="!isInWatchlist(coin)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
              ...
              <td class="py-4 px-4">
                <div class="flex items-center gap-3">
                    <div class="relative w-8 h-8 flex-shrink-0">
                         <!-- Image Icon -->
                         <img [src]="'https://assets.coincap.io/assets/icons/' + (coin.imageKey || coin.symbol || coin.name).toLowerCase() + '@2x.png'" 
              ...
              <td class="py-4 px-4 text-right">
                  <div [class.text-emerald-400]="coin.change >= 0" [class.text-red-400]="coin.change < 0" class="font-mono text-sm font-medium">
...
export class MarketListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.watchlistSubscription = this.marketService.watchlist$.subscribe(list => {
      this.watchlist = list;
    });
  }
  ...
  sortData(data: any[]) {
    return [...data].sort((a, b) => {
      ...
      if (['priceUsd', 'volumeUsd24Hr', 'changePercent24Hr', 'marketCapUsd'].includes(this.sortColumn)) {
